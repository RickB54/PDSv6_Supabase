...
const subtotalRegex = /<div className="flex justify-between items-center px-2">\s*<span className="text-xs text-zinc-500 font-bold uppercase tracking-wider">Subtotal Owed<\/span>\s*<span className="text-xl font-bold text-emerald-500">\$[^<]*<\/span>\s*<\/div>/;
...
if (subtotalRegex.test(content)) {
  content = content.replace(subtotalRegex, ui_new_code);
