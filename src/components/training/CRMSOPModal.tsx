  ...
  const handleDownloadPDF = () => {
    try {
      ...
      const addPageHeader = (pageNum: number) => {
        ...
        doc.setTextColor(185, 28, 28); // Red brand
        doc.text('PRIME AUTO DETAIL', pageWidth / 2, 12, { align: 'center' });
        
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(15, 23, 42);
        doc.text('CRM Standard Operating Procedures', pageWidth / 2, 18, { align: 'center' });

        doc.setFontSize(8);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(100, 116, 139);
        doc.text('Customer Profile Lookup • Job Booking Channels • Activity Logging • Duplicate Cleanup', pageWidth / 2, 23, { align: 'center' });
      ...
      const checkPageBreak = (needed: number) => {
        if (currentY + needed > pageHeight - 15) {
          doc.addPage();
          currentY = addPageHeader(doc.getNumberOfPages());
        }
      };
      ...
      doc.text('CORE PRINCIPLE: "Search first. Book from the card. Confirm the Booking Lifecycle panel. Every time, no exceptions."', 14, currentY, { maxWidth: pageWidth - 28 });
      currentY += 10;
      ...
      const rules = [
        '• Always search before booking. Go to Customer Profiles and search by name, phone, or email before creating any booking. This prevents duplicate records.',
        '• Always book from the customer card. Use the green Book Job button on the customer\'s card — never go to the Bookings page directly for a known customer. Booking from the card pre-fills their exact database ID, guaranteeing a perfect link every time.',
        '• Booking Lifecycle panel is your confirmation. After any booking is created, check the Booking Lifecycle panel on the right side of the customer card. If it still says "No Booking Data Yet," the booking is not linked correctly.',
        '• Log Activity is your default. For 90% of daily interactions (calls, texts, voicemails, notes), use Log Activity on the customer card. One line is enough.'
      ];
      ...
      doc.text('Channel 1 — Phone Call Intake', 14, currentY);
      currentY += 6;

      doc.setFontSize(9.5);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(15, 23, 42);
      doc.text('Recommended Flow (Best Way)', 14, currentY);
      currentY += 5;

      const ch1Steps = [
        '• Go to Customer Profiles -> search by name, phone, or email',
        '• If found -> open their card. If not found -> click + Add, create profile, save it',
        '• On their card -> tap Log Activity -> enter a quick note (e.g. "Inbound call — wants full interior Thursday")',
        '• On their card -> tap the green Book Job button',
        '• Fill in date, service, vehicle, price -> add a booking note for anything job-specific -> Save',
        '• Verify the Booking Lifecycle panel on the right shows the job'
      ];
      doc.setFontSize(8.5);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      ch1Steps.forEach(s => {
        ...
        checkPageBreak(split.length * 4 + 1);
        doc.text(split, 14, currentY);
      ...
      doc.text('Alternative Flow (Rush Only — Use with Caution)', 14, currentY);
      currentY += 5;

      const ch1Rush = [
        '• Go directly to Admin Bookings page -> create booking manually',
        '• System will auto-match by Email -> Name -> Phone',
        '• If it matches on any one field, it links correctly',
        '• WARNING: If customer used different name/email/phone, a duplicate Prospect record will be created. Do NOT use for repeat customers.'
      ];
      doc.setFontSize(8.5);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(30, 58, 138);
      doc.text('The Rule: Search -> open card -> Log Activity -> Book Job button -> confirm Booking Lifecycle panel', 14, currentY + 3);
      currentY += 10;
      ...
      doc.text('Channel 2 — Website / Email Inquiry', 14, currentY);
      currentY += 6;
      ...
      doc.setTextColor(51, 65, 85);
      const ch2Steps = [
        '• Read inquiry fully — note name, vehicle, service interest, date preference, contact info.',
        '• Go to Customer Profiles -> search by name AND by email',
        '• If found -> open card. If not -> create new',
        '• Tap Log Activity -> note inquiry (e.g. "Inbound email — wants full detail, prefers Thursday")',
        '• Reply via Engagement Hub — offer 1-2 available time slots (auto-logs reply)',
        '• Once customer confirms -> tap green Book Job button on card -> Fill in job details -> save',
        '• Tap Log Activity again -> "Customer confirmed — booked for [date]"'
      ];
      ch2Steps.forEach(s => {
        const split = doc.splitTextToSize(s, pageWidth - 28);
        checkPageBreak(split.length * 4 + 1);
      ...
      doc.text('Channel 3 — Direct / Admin Booking', 14, currentY + 3);
      currentY += 9;

      const ch3Steps = [
        '• Go to Customer Profiles -> search first, even if you are certain who it is',
        '• Open their card -> tap green Book Job button',
        '• Fill in service, date, vehicle, price -> add booking note if anything unusual applies -> Save',
        '• Verify Booking Lifecycle panel shows the job',
        '• Tap Log Activity -> note how booking came in (e.g. "Walk-in", "Outbound call")'
      ];
      doc.setFontSize(8.5);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      ...
      doc.text('Channel 4 — Online Booking (Automatic)', 14, currentY + 3);
      currentY += 9;
      ...
      doc.setTextColor(51, 65, 85);
      const ch4Steps = [
        '• System runs 3-tier cascade match: Tier 1 (Exact Email), Tier 2 (Name), Tier 3 (Phone).',
        '• If match succeeds: booking links automatically. Verify calendar landing & Lifecycle panel.',
        '• If "No Booking Data Yet" duplicate occurs: Open booking -> Edit -> Select Customer -> pick master profile -> save. Then delete duplicate prospect.'
      ];
      ch4Steps.forEach(s => {
        const split = doc.splitTextToSize(s, pageWidth - 28);
        checkPageBreak(split.length * 4 + 1);
      ...
      // Timeline Tabs & Cleanup
      checkPageBreak(60);
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(15, 23, 42);
      doc.text('Customer Card Guide & Cleanup Procedure', 14, currentY + 3);
      currentY += 9;

      const guidePoints = [
        '• Timeline Tabs: All Data (all bookings + engagements), Bookings (only bookings), Notes (Admin Directives), Correspondences (Engagement Hub emails), Activities (Log Activity touchpoints).',
        '• Order of Operations: 1. Check Admin Directives -> 2. Glance at Booking Lifecycle panel -> 3. Keep timeline on All Data -> 4. Tap Log Activity for touchpoints -> 5. Use Engagement Hub for formal emails -> 6. Always book via Book Job button.',
        '• Duplicate Cleanup: Open orphaned booking -> Edit -> Select Customer -> choose master profile -> save. Go to Customer Profiles -> delete duplicate Prospect.'
      ];
      doc.setFontSize(8.5);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      guidePoints.forEach(p => {
        const split = doc.splitTextToSize(p, pageWidth - 28);
        checkPageBreak(split.length * 4 + 1);
        doc.text(split, 14, currentY);
        currentY += split.length * 4 + 1;
      });
      ...
      doc.setTextColor(100, 116, 139);
      doc.text('Prime Auto Detail • Precision. Protection. Perfection. | June 2026 (Version 3)', pageWidth / 2, pageHeight - 10, { align: 'center' });

      doc.save('CRM_Standard_Operating_Procedures_v3.pdf');
      toast({ title: 'PDF Exported', description: 'Complete CRM Standard Operating Procedures downloaded successfully.' });
    } catch (e: any) {
