...
export const HeadlightRestorationGuideModal: React.FC<HeadlightRestorationGuideModalProps> = ({ open, onOpenChange }) => {
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('Headlight Restoration Employee Training Guide', pageWidth / 2, currentY, { align: 'center' });
      currentY += 6;
      ...
      const purp = doc.splitTextToSize('This guide gives employees a consistent way to assess headlight condition, perform safe restoration, and set accurate customer expectations about how long results last.', pageWidth - 32);
      ...
      doc.text('CORE PRINCIPLE: "Restoration renews the surface. It doesn\'t fix what\'s happening inside the housing."', pageWidth / 2, currentY, { align: 'center' });
      currentY += 5;
      doc.setFontSize(8);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(185, 28, 28);
      doc.text('1. Why Headlights Oxidize', 14, currentY);
      currentY += 6;
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      const s1 = doc.splitTextToSize('Most modern headlight lenses are polycarbonate plastic with a factory UV-protective clear coat. Over time, sun exposure breaks down that clear coat, allowing the polycarbonate underneath to oxidize — causing the hazy, yellowed appearance customers ask about.', pageWidth - 28);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(185, 28, 28);
      doc.text('2. Assessing Headlight Condition', 14, currentY);
      currentY += 6;

      const conds = [
        '• Light surface haze: Yes — often polish-only, no sanding needed',
        '• Moderate-to-deep yellowing: Yes — full wet-sand progression needed',
        '• Cracked lens: No — restoration won\'t fix structural damage; recommend replacement',
        '• Foggy/moisture inside the housing: No — indicates a seal failure; polishing the outside won\'t help; recommend replacement or seal repair'
      ];
      conds.forEach(c => {
      ...
      const rule2 = doc.splitTextToSize('Employee rule: Always assess before quoting. Selling restoration on a lens that\'s cracked or fogged internally sets the customer up for disappointment and reflects badly on the shop.', pageWidth - 30);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(185, 28, 28);
      doc.text('3. Tools & Materials', 14, currentY);
      currentY += 6;

      const tools = [
        '• Progressive wet-sand grits (coarse -> medium -> fine, commonly 800-3000 range)',
        '• Buffing/polishing compound for post-sand clarity.',
        '• UV-protective sealant or coating — this step is NOT optional; skipping it means oxidation returns within weeks.',
        '• Painter\'s tape/masking to protect surrounding paint and trim from sanding debris and compound.'
      ];
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      tools.forEach(t => {
        const split = doc.splitTextToSize(t, pageWidth - 28);
        checkPageBreak(split.length * 4.5 + 2);
        doc.text(split, 14, currentY);
        currentY += split.length * 4.5 + 2;
      });
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(185, 28, 28);
      doc.text('4. Step-by-Step Restoration Process', 14, currentY + 4);
      currentY += 10;

      const steps = [
        '• Mask off all surrounding paint, trim, and badges — sanding debris and compound can dull or scratch adjacent paint if not protected.',
        '• Wet sand progressively through each grit, from coarsest to finest, keeping the surface wet throughout to avoid heat buildup and deeper scratching. Do not skip a grit step — each one is there to remove the scratch pattern left by the previous one.',
        '• Once sanding is complete and the surface is uniformly hazy/matte (not clear yet), compound and polish the lens to restore clarity.',
        '• Clean and fully dry the lens.',
        '• Apply the UV-protective coating per the product\'s exact cure instructions — this is what prevents oxidation from returning quickly.'
      ];
      steps.forEach(s => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(185, 28, 28);
      doc.text('5. Customer Expectations & Quick Reference Rules', 14, currentY + 4);
      currentY += 10;

      const qRules = [
        '• Always assess for cracks or internal fogging before quoting — these aren\'t restoration candidates.',
        '• Always mask surrounding paint/trim before sanding.',
        '• Never skip a grit step in the sanding progression.',
        '• Always apply the UV coating — never skip this step.',
        '• Always respect the coating\'s cure time before customer pickup.'
      ];
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      ...
      doc.save('Prime_Auto_Detail_Headlight_Restoration_Employee_Training_Guide.pdf');
      toast({ title: 'PDF Exported', description: 'Headlight Restoration Employee Training Guide downloaded.' });
    } catch (e: any) {
