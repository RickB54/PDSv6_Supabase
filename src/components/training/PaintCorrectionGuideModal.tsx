...
export const PaintCorrectionGuideModal: React.FC<PaintCorrectionGuideModalProps> = ({ open, onOpenChange }) => {
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('Paint Correction Employee Training Guide', pageWidth / 2, currentY, { align: 'center' });
      currentY += 6;

      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(100, 116, 139);
      doc.text('Employee Training • Paint Inspection • Correction Process • Quality Control', pageWidth / 2, currentY, { align: 'center' });
      currentY += 10;

      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(185, 28, 28);
      doc.text('CORE PRINCIPLE: "Correction removes material to level a defect. Every pass costs clear coat you cannot put back."', 14, currentY, { maxWidth: pageWidth - 28 });

      doc.save('Prime_Paint_Correction_Training_Guide.pdf');
      toast({ title: 'PDF Exported', description: 'Paint Correction Employee Training Guide downloaded.' });
    } catch (e: any) {
