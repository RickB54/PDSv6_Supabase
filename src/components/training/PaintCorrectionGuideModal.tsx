...
export const PaintCorrectionGuideModal: React.FC<PaintCorrectionGuideModalProps> = ({ open, onOpenChange }) => {
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      const purp = doc.splitTextToSize('This guide gives employees a consistent, professional way to assess paint defects, perform safe machine correction, and set accurate customer expectations. Paint correction is the highest-skill, highest-risk service Prime Auto Detail offers.', pageWidth - 32);
      ...
      doc.text('CORE PRINCIPLE: "Correction removes material to level a defect. Every pass costs clear coat you cannot put back."', pageWidth / 2, currentY, { align: 'center', maxWidth: pageWidth - 28 });
      currentY += 8;
      doc.setFontSize(8);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(180, 83, 9);
      doc.text('1. What Paint Correction Is — and Isn\'t (PC-01)', 14, currentY);
      currentY += 6;
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(51, 65, 85);
      const s1 = doc.splitTextToSize('Paint correction levels the clear coat to reduce or remove swirl marks, holograms, light scratches, water etching, and oxidation. It works by removing a microscopically thin layer of clear coat until the surrounding area is level with the bottom of the defect.', pageWidth - 28);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(180, 83, 9);
      doc.text('2. Clear Coat & Safe Removal Limits (PC-01, PC-02)', 14, currentY);
      currentY += 6;
      const ccLimits = [
        '• Clear coat thickness averages 1.5-2 mils (thousandths of an inch). Factory variance, repaints, and edges can be thinner.',
        '• Edges, body lines, and high points always have thinner clear coat than flat panel centers — extra caution required.',
        '• Repainted panels may have unknown or reduced clear coat — treat with extra caution or decline correction until verified.'
      ];
      ccLimits.forEach(c => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(180, 83, 9);
      doc.text('3. Compounding vs. Polishing vs. Finishing (PC-04, PC-06)', 14, currentY + 4);
      currentY += 10;
      const stages = [
        '• Compounding (cutting): Removes heavier defects — deeper swirls, oxidation, moderate scratches. Most aggressive.',
        '• Polishing: Refines surface after compounding, removes lighter marring. Medium aggressiveness.',
        '• Finishing: Final refinement pass for maximum gloss and clarity. Least aggressive.'
      ];
      stages.forEach(s => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(180, 83, 9);
      doc.text('4. Step-by-Step Correction Process (PC-01 to PC-10)', 14, currentY + 4);
      currentY += 10;
      const steps = [
        '• Fully wash, decontaminate (iron removal), and clay bar before machine work.',
        '• Tape off trim, rubber seals, badges, and plastic body cladding.',
        '• Perform a test spot on a small, inconspicuous area first.',
        '• Work in 2x2 ft sections with overlapping crosshatch passes.',
        '• Inspect section under dedicated 45-degree light before moving on.',
        '• Perform IPA panel wipe to remove polishing oils before applying protection.'
      ];
      steps.forEach(s => {
        const split = doc.splitTextToSize(s, pageWidth - 28);
        checkPageBreak(split.length * 4.5 + 2);
      ...
      // Customer Script & Rules
      checkPageBreak(50);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(180, 83, 9);
      doc.text('5. Customer Expectations & Quick Reference Rules (PC-09, PC-10)', 14, currentY + 4);
      currentY += 10;

      const qRules = [
        '• Always wash, decon, and clay before machine work.',
        '• Always test spot before committing to full panel.',
        '• Always work most-aggressive to least-aggressive.',
        '• Never cross-contaminate pads between compound tiers.',
        '• Always inspect under dedicated light, not ambient shop lighting.',
        '• Always IPA wipe-down before applying protection product.'
      ];
      qRules.forEach(r => {
      ...
      doc.save('Prime_Auto_Detail_Paint_Correction_Employee_Training_Guide.pdf');
