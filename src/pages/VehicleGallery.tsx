...
import {
    ...
    Calendar, Filter, Share2, Facebook, Copy
} from "lucide-react";
import { getCurrentUser } from "@/lib/auth";
import { getSupabaseCustomers, Customer, Vehicle, upsertSupabaseVehicle, supabase } from "@/lib/supa-data";
...
export default function VehicleGallery() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            // Filter out customers with no vehicles or no media? 
            // Better to show all and allow adding.
            setCustomers(data);

            // Auto-expand if only a few customers or if deep linking (future)
            if (data.length < 5) {
                setExpandedCustomers(data.map(c => c.id || ""));
            }
        } catch (err) {
    ...
    const handleDeleteMedia = async (vehicle: Vehicle, type: 'general' | 'before' | 'after' | 'video', index: number) => {
        ...
        try {
            ...
            toast({ title: "Media Removed", description: "The item has been deleted successfully." });
            loadData(); // Refresh
        } catch (err) {
            console.error("Failed to delete media:", err);
            toast({ title: "Delete Failed", description: "Could not remove the media item.", variant: "destructive" });
        }
    };
                ...
                <div key={idx} className="group relative aspect-square rounded-lg bg-zinc-950 border border-zinc-800 overflow-hidden hover:border-zinc-600 transition-all shadow-lg">
                    {type === 'image' ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center bg-zinc-900">
                            <div className="h-10 w-10 rounded-full bg-pink-600/20 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                                <Video className="h-5 w-5 text-pink-500" />
                            </div>
                            <span className="text-[10px] text-zinc-500 font-mono truncate px-2 w-full text-center">
