...
function isMenuHidden(key: string): boolean {
...
export default function AdminDashboard() {
  ...
  const loadUsers = async () => {
    try {
      ...
      // Combine: Start with profiles, then add invites that don't have a profile yet
  ...
  const loadEmployees = async () => {
    try {
      ...
      const { data: invites } = await supabase.from('authorized_users').select('*').eq('role', 'employee');

      const activeEmails = new Set((profiles || []).map(p => p.email));

      // Combine lists: Active profiles + Invites that haven't signed up yet
      const pending = (invites || []).filter(i => !activeEmails.has(i.email)).map(i => ({
        ...i,
        id: 'pending_' + i.email,
        status: 'Pending Signup'
      }));

      setEmployees([...(profiles || []), ...pending]);
    } catch { setEmployees([]); }
  };
  ...
  const createEmployee = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('authorized_users').insert({
        ...
        name: empNewName,
        role: 'employee',
        added_by: user?.email // simplistic audit
      });
      ...
      await loadEmployees();
      toast({ title: 'Employee Authorized', description: 'They can now sign up with this email.' });
    } catch (err: any) {
      toast({ title: 'Failed to authorize', description: err.message, variant: 'destructive' });
    }
  };
  ...
  const handleCreateUser = async () => {
    try {
      ...
      toast({ title: 'User Authorized', description: `${payload.name} can now sign up as ${payload.role}.` });
      setNewName(''); setNewEmail(''); setNewPassword(''); setNewRole('employee');
      await loadUsers();

    } catch (err: any) {
      toast({ title: 'Failed to authorize', description: err.message || 'Unexpected error' });
    }
  };
  ...
  const handleUpdateRole = async (id: string, role: 'employee' | 'admin') => {
    try {
      ...
      // 2. Update app_users if they have a profile
