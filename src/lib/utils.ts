...
/**
 * Converts any date string or Date object into "YYYY-MM-DD" string for HTML date inputs (<input type="date">).
 */
export function toInputDateFormat(dStr: string | Date | undefined | null): string {
  if (!dStr) return '';
  const d = parseLocalDate(dStr);
  if (isNaN(d.getTime())) return '';
