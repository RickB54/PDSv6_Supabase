...
export const getCustomerDetailedHistory = async (customerId: string) => {
    return appCache.fetchWithCache('customers', `customer_360_${customerId}`, async () => {
        try {
    ...
    try {
        ...
        const payload: any = {
            ...
            booking_vehicle: {
              ...
              is_archived: Boolean(booking.isArchived || (booking as any).is_archived || false),
              isArchived: Boolean(booking.isArchived || (booking as any).is_archived || false)
            },
