...
// Deno Deploy Edge Function: create admin via Supabase Admin API
...
serve(async (req: Request) => {
    ...
    try {
        const { error } = await supabase
            .from('app_users')
            .upsert({ id: createdUserId, email, role: 'admin', name, is_active: true, updated_at: new Date().toISOString() }, { onConflict: 'id' });
        if (error) throw error;
    } catch (e) {
        return Response.json({ ok: false, error: `app_users_upsert_failed:${String((e as any)?.message || e)}` }, { status: 500 });
    }

    return Response.json({ ok: true, user: { id: createdUserId, email, name, role: 'admin' } });
});
